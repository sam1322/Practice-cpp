...
class Graph{
    ...
    Graph(vector<edge> edges , int N ){
        ...
        this->N = N ; 
        for(auto &p : edges){
            adj[p.src].push_back(mk( p.wt , p.dest)) ; 
            adj[p.dest].push_back(mk(p.wt , p.src) )  ; 
        }
    }
};
...
void prim(Graph &g , int src) {
    ...
    while(!q.empty()){
        ...
        parents[p.second.first] = p.second.second ; 

        for(auto &y:g.adj[x]){ 

            if(!vis[y.second]){
                piii Y = { y.first , {y.second , x  }} ; 
                q.push(Y) ;
            }
