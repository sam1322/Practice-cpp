...
// time complexity O(N) and space complexity of morris inorder traversal is 0(1)
...
bool check(Node *root){
    ...
    bool l = 1 ,r  = 1 ;
    if(root->left){
        l =(( root->data > root->left->data)  && check(root->left) );
    }
    if(root->right){
        r = ((root->data > root->right->data) && check(root->right)) ; 
    }
    return l&&r ; 
...
void solve(){
    ...
    cout<<endl;
    vector<Node*> v;
    inorder(root,v);fr(i,v.size())cout<<v[i]->data<<" ";cout<<endl;
    inorder_iter(root);cout<<endl;
    inorder_iter2(root);cout<<endl;
    morris_inorder_traversal(root);cout<<endl;
